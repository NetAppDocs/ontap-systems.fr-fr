...
summary: 'Pour assurer la sécurité des données sur votre système de stockage AFF C80, vous devez vérifier la prise en charge et l"état de la clé de cryptage. Vérifiez si votre version de ONTAP prend en charge NetApp Volume Encryption (NVE) et avant d"arrêter le contrôleur, vérifiez si le gestionnaire de clés est actif.' 
---
= Vérifiez la prise en charge et l'état de la clé de cryptage sur le support de démarrage - AFF C80
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
