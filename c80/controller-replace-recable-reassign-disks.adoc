...
[role="lead"]
Replacez le contrôle des ressources de stockage sur le contrôleur de remplacement afin que votre système AFF C80 puisse reprendre son fonctionnement normal. La procédure de retour à zéro varie en fonction du type de chiffrement utilisé par votre système : pas de chiffrement, chiffrement OKM (Onboard Key Manager) ou chiffrement EKM (External Key Manager).
