...
summary: 'Pour garantir la sécurité des données sur votre AFF A70 ou AFF A90, vous devez vérifier la prise en charge et l"état de la clé de chiffrement sur votre support de démarrage. Vérifiez si votre version d"ONTAP prend en charge NetApp Volume Encryption, et avant d"arrêter le contrôleur, vérifiez si le gestionnaire de clés est actif.' 
---
= Vérifier la prise en charge du cryptage pour la récupération manuelle du support de démarrage - AFF A70 et AFF A90
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
