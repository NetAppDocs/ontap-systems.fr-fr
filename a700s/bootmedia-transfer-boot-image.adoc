...
summary: 'Transférez l"image de démarrage sur le support de démarrage de remplacement d"un système AFF A700s en utilisant soit le support de démarrage secondaire, soit une clé USB. Le système AFF A700s ne prend en charge que les procédures de récupération manuelles du support de démarrage. La récupération automatique du support de démarrage n"est pas prise en charge.' 
---
...
[role="lead"]
Transférez l'image de démarrage sur le support de démarrage de remplacement d'un AFF A700s en utilisant soit le support de démarrage secondaire (méthode principale), soit une clé USB (si la restauration du support de démarrage secondaire échoue ou si le fichier  `image.tgz` est manquant).
...
== Option 1 : Transférer l’image système à l’aide d’une sauvegarde de récupération depuis le second support de démarrage

Vous pouvez installer l'image système sur le support de démarrage de remplacement en utilisant l'image présente sur le second support de démarrage dans le module contrôleur. Il s'agit de la méthode principale pour les systèmes disposant de deux supports de démarrage dans le module contrôleur.

L'image sur le support de démarrage secondaire doit contenir un  `image.tgz` fichier et ne doit pas signaler d'erreurs. Si le  `image.tgz` fichier est manquant ou si le support de démarrage signale des erreurs, vous ne pouvez pas utiliser cette procédure. Utilisez <<Option 2 : Transférer l’image de démarrage à l’aide d’une clé USB>> à la place.
...
== Option 2 : Transférer l’image de démarrage à l’aide d’une clé USB

Cette procédure ne doit être utilisée que si la restauration du support de démarrage secondaire a échoué ou si le  `image.tgz` fichier est introuvable sur le support de démarrage secondaire.

* Vous devez disposer d'une clé USB, formatée en FAT32, avec au moins 4 Go de capacité.
* Une copie de la même version d'image ONTAP que celle utilisée par le contrôleur défectueux. Vous pouvez télécharger l'image appropriée dans la section Téléchargements du NetApp Support Site.
+
...
.. Enregistrez les variables d'environnement que vous avez modifiées avec la commande  `savenv`.
...
. À partir du contrôleur partenaire au niveau de privilège avancé, démarrez la synchronisation de la configuration à l'aide de l'adresse IP enregistrée à l'étape précédente : `system node restore-backup -node local -target-address _impaired_node_IP_address_`
. Une fois la synchronisation de la configuration terminée sans erreur, appuyez sur `y` lorsque vous êtes invité à confirmer que la procédure de sauvegarde a réussi.
. Vérifiez que les variables d'environnement sont configurées comme prévu.
+
