...
[role="lead"]
Transférez l'image de démarrage sur le support de démarrage de remplacement d'un système AFF A700s en utilisant soit le support de démarrage secondaire, soit une clé USB. Cette procédure consiste à restaurer l'image sur le support de démarrage secondaire comme méthode principale, ou à utiliser une clé USB si la restauration sur le support de démarrage secondaire échoue ou si le fichier image.tgz est manquant.
...
== Option 1 : Transférer les fichiers à l’aide d’une sauvegarde de récupération depuis le second support de démarrage
