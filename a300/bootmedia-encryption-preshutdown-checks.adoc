...
summary: 'Avant de mettre hors service le contrôleur défectueux et de vérifier l"état des clés de chiffrement intégrées, vous devez vérifier l"état du contrôleur défectueux, désactiver la restitution automatique et vérifier la version d" ONTAP exécutée par le système. Le système AFF A300 ne prend en charge que les procédures de récupération manuelles du support de démarrage.' 
---
...
[role="lead"]
Pour garantir la sécurité des données sur votre système de stockage, vous devez vérifier la prise en charge et l'état de la clé de chiffrement sur votre support de démarrage. Le système AFF A300 ne prend en charge que les procédures de récupération manuelles du support de démarrage. La récupération automatique du support de démarrage n'est pas prise en charge.

Vérifiez si votre version ONTAP prend en charge le chiffrement de volume NetApp (NVE), et avant d'arrêter le contrôleur, vérifiez si le gestionnaire de clés est actif.
