...
summary: 'Remplacez la NVDIMM dans votre module contrôleur AFF A400 lorsque le système détecte que la durée de vie de la mémoire flash est presque terminée ou que la NVDIMM identifiée n"est pas saine. Le non-remplacement d"une NVDIMM défectueuse peut provoquer une panique du système.' 
---
= Remplacez un NVDIMM - AFF A400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
