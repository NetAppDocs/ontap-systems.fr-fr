...
summary: 'Après avoir terminé le remplacement de l"unité remplaçable sur site (FRU) dans une configuration MetroCluster à deux nœuds, effectuez l"opération de basculement MetroCluster pour rétablir la configuration dans son état de fonctionnement normal, les machines virtuelles de stockage de la source de synchronisation sur le site précédemment défaillant étant désormais actives et fournissant des données à partir des pools de disques locaux.' 
---
= Retournez des agrégats dans une configuration MetroCluster à deux nœuds - AFF A400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
