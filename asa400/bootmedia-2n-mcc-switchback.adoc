...
summary: 'Après avoir terminé le remplacement de la FRU dans une configuration MetroCluster à deux nœuds, vous pouvez effectuer l"opération de basculement MetroCluster. Cela ramène la configuration à son état de fonctionnement normal, avec les machines virtuelles de stockage SVM source de synchronisation sur le site précédemment défaillant désormais actives et servant les données à partir des pools de disques locaux.' 
---
= Basculez les agrégats dans une configuration MetroCluster à deux nœuds - ASA A400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
