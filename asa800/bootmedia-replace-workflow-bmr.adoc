...
summary: 'Commencez à remplacer le support de démarrage de votre système de stockage ASA A800 en examinant les exigences de remplacement, en arrêtant le contrôleur, en remplaçant le support de démarrage, en restaurant l"image sur le support de démarrage et en vérifiant la fonctionnalité du système.' 
---
= Workflow de récupération automatisé du support de démarrage - ASA A800
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
La récupération automatique de l'image de démarrage implique que le système identifie et sélectionne automatiquement l'option de menu de démarrage appropriée. Il utilise l'image de démarrage du nœud partenaire pour réinstaller ONTAP sur le support de démarrage de remplacement de votre système ASA A800 .
