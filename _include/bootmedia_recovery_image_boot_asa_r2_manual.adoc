...
--
.. À l'invite Loader du contrôleur défectueux, configurez l'adresse IP du module de contrôleur défectueux :
+
`ifconfig e0M - auto`

.. À l'invite Loader, netboot le nœud défectueux :
+
`netboot \http://_web_server_ip/path_to_web-accessible_directory_/<xxx_image.tgz>`

.. Dans le menu de démarrage, sélectionnez option `7`.
.. Entrez `n` lorsque vous êtes invité à restaurer la configuration de sauvegarde.
.. Entrez `y` lorsque vous êtes invité à redémarrer le nœud.
.. Appuyez sur `ctrl-c` pour revenir au menu de démarrage.
+
Si le chiffrement est activé, vous devez le restaurer avant de continuer à restaurer l'image de démarrage. Allez à link:bootmedia-encryption-restore.html["Restaurez le chiffrement"].

.. Sur le contrôleur défectueux, appuyez sur `Y` lorsque vous êtes invité à redémarrer le contrôleur défectueux et appuyez sur `ctrl-c` pour accéder au menu de démarrage.
.. Sélectionnez option `6`.
