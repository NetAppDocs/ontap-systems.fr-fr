...
. Depuis le nœud sain, restaurez le rétablissement automatique si vous l'avez désactivé à l'aide de la `storage failover modify -node local -auto-giveback true` commande.
...
. Remettez le contrôleur en place à partir du contrôleur partenaire :`stockage failover giveback -ofnode target_node_name`
