...
summary: 'Pour garantir la sécurité des données sur votre système de stockage FAS70 ou FAS90, vous devez vérifier la prise en charge et l"état de la clé de chiffrement sur votre support de démarrage. Vérifiez si votre version ONTAP prend en charge le chiffrement de volume NetApp et, avant d"arrêter le contrôleur, si le gestionnaire de clés est actif.' 
---
= Vérifier la prise en charge du chiffrement pour la récupération manuelle du support de démarrage - FAS70 et FAS90
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
