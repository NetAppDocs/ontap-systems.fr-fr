...
summary: 'Une fois le remplacement de l"unité remplaçable sur site (FRU) terminé dans une configuration MetroCluster à deux nœuds, vous pouvez effectuer l"opération de basculement MetroCluster. Cette opération rétablit la configuration dans son état de fonctionnement normal, les machines virtuelles de stockage source synchronisée du site précédemment défaillant étant désormais actives et fournissant des données à partir des pools de disques locaux.' 
---
= Recommutation des agrégats dans une configuration MetroCluster à deux nœuds - FAS8300 et FAS8700
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
