...
+
** Si keymanager, NSE ou NVE intégré est configuré sur votre système, rendez-vous sur xref:bootmedia_encryption_restore.adoc[OKM, NSE et NVE si besoin]
...
. À l'invite du cluster, vérifiez les interfaces logiques avec le `net int show -is-home false` commande.
+
Si l'une des interfaces est indiquée comme « FALSE », restaurez ces interfaces à son port d'origine à l'aide de l' `net int revert -vserver _vserver_name_ -lif _lif_name_` commande.
...
. Si vous n'utilisez pas le chiffrement du stockage, restaurez le rétablissement automatique et AutoSupport :
+
