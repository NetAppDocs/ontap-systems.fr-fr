...
|===
. Repérez le module de voyants USB à l'avant de l'ancien châssis, directement sous les modules DCPM.
...
+
.. Au fur et à mesure que chaque nœud démarre, appuyez sur `Ctrl-C` Pour interrompre le processus de démarrage lorsque le message s'affiche, appuyez sur Ctrl-C pour le menu de démarrage.
+
