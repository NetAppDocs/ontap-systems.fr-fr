...
---
= Remplacer le contrôleur - ASA C250
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Remplacez le contrôleur de votre système ASA C250 lorsqu'une défaillance matérielle l'exige. La procédure de remplacement consiste à retirer le contrôleur défectueux, à transférer les composants vers le contrôleur de remplacement, à installer le contrôleur de remplacement et à le redémarrer.
...
.Et la suite ?
Après avoir remplacé le contrôleur ASA C250 défectueux, vous devez link:controller-replace-system-config-restore-and-verify.html["restaurer et vérifier la configuration du système"].
