...
summary: Pour que le remplacement du contrôleur de votre système ASA C250 soit pleinement opérationnel, vous devez restaurer la configuration de chiffrement du stockage NetApp si nécessaire, installer les licences pour le nouveau contrôleur et renvoyer la pièce défectueuse à NetApp, comme décrit dans les instructions RMA fournies avec le kit. 
---
= Remplacement complet du contrôleur - ASA C250
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Pour que le remplacement du contrôleur de votre système ASA C250 soit pleinement opérationnel, vous devez restaurer la configuration de chiffrement du stockage NetApp (si nécessaire), installer les licences pour le nouveau contrôleur et renvoyer la pièce défectueuse à NetApp, comme décrit dans les instructions RMA fournies avec le kit.
...
Avant de remettre le nœud de remplacement en service, vous devez vérifier que les LIF sont sur leurs ports d'origine, enregistrer le numéro de série du nœud de remplacement si AutoSupport est activé, vérifier l'état de votre cluster, fermer toutes les fenêtres de maintenance et réinitialiser la remise en service automatique.
