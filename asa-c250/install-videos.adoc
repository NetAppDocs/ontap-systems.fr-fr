---
permalink: asa-c250/install-videos.html 
sidebar: sidebar 
keywords: asa c250, hardware installation and cabling video, performing end-to-end software configuration video 
summary: 'Il y a deux vidéos : l’une montre comment installer et câbler votre système, et l’autre présente un exemple d’utilisation de System Manager Guided Setup pour effectuer la configuration initiale du système.' 
---
= Vidéos - ASA C250
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
La vidéo suivante montre comment installer et raccorder votre nouveau système.

.Animation - installation et configuration d'un AFF A250
video::fe6876d5-9332-4b2e-89be-ac6900027ba5[panopto]

NOTE: Le ASA C250 utilise la même procédure d'installation que le système AFF A250.
