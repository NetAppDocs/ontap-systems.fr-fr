...
summary: 'Lors du remplacement d"un tiroir disque dans un tiroir, vous pouvez soit arrêter l"ensemble du système de stockage (paire haute disponibilité), soit maintenir la paire haute disponibilité en fonctionnement.' 
---
= Remplacer un tiroir disque DS460C
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Cette procédure s'applique aux tiroirs disques DCM et/ou aux tiroirs disques DCM2 ou DCM3. (Les tiroirs disques disposent également de deux modules IOM12, deux modules IOM12B ou deux modules IOM12C.)
+
...
[NOTE]
====
Lorsque vous remplacez un tiroir de lecteur défectueux par un tiroir de lecteur DCM2 ou DCM3 plus récent, assurez-vous que le logiciel ONTAP et le micrologiciel du module IOM12, IOM12B ou IOM12C exécutent les versions minimales requises pour prendre en charge les tiroirs DCM2 ou DCM3.

La mise à jour du firmware IOM12, IOM12B ou IOM12C peut être effectuée avant ou après le remplacement d'un tiroir disque. Cette procédure vous demande de mettre à jour le firmware dans le cadre de la préparation à la procédure de remplacement du tiroir disque.
...
. Si votre tiroir comprend une combinaison de modules IOM12, IOM12B ou IOM12C avec un nombre quelconque de tiroirs de disques DCM3, votre système doit exécuter les versions minimales suivantes d'ONTAP :
+
** ONTAP 9.17.1RC1
** ONTAP 9.16.1P3
** ONTAP 9.15.1P11
