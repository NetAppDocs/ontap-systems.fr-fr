...
summary: 'Utilisez cet exemple pour vous guider dans la lecture et l"application d"une feuille de travail remplie pour connecter le contrôleur à la pile pour les tiroirs disques afin d"assurer une connectivité multipathed.' 
---
= Consultez la fiche de câblage pour les connexions multivoies contrôleur-vers-pile DS212C, DS224C ou DS460C
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Vous pouvez utiliser cet exemple pour vous guider dans la lecture et l'application d'une feuille de travail remplie afin de connecter le contrôleur à la pile pour les tiroirs disques équipés de modules IOM12, IOM12B ou IOM12C, afin d'assurer une connectivité multipathed.
...
+
La configuration utilisée dans cet exemple est une configuration haute disponibilité multipath avec deux cartes HBA SAS quad-port (huit ports SAS) sur chaque contrôleur et deux piles de tiroirs disques équipées de modules IOM12, IOM12B ou IOM12C. Les paires de ports sont câblées en sautant une paire sur deux dans la feuille de calcul.
