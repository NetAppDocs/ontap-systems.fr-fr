...
---
= Ajouter un module d'E/S - AFF C30 et AFF C60
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Ajoutez un module d'E/S à votre système de stockage AFF C30 ou AFF C60 pour améliorer la connectivité réseau et augmenter la capacité de traitement du trafic de données. Vous pouvez utiliser cette procédure pour ajouter un module d'E/S soit à un contrôleur opérationnel dans le cadre d'une mise à niveau planifiée, soit à un contrôleur défaillant lors d'une opération de maintenance.
...
Vous pouvez effectuer cette procédure sur l'un ou l'autre des contrôleurs de la paire haute disponibilité, qu'il soit actuellement opérationnel ou défaillant. Comme il est impossible d'ajouter un module d'E/S à un contrôleur en fonctionnement, le contrôleur cible doit être arrêté avant l'installation du module. Le partenaire de la paire haute disponibilité prend le relais des opérations de stockage pendant la procédure afin que les données restent accessibles aux clients.

* Si le contrôleur cible est *opérationnel* (ajout prévu), suivez les étapes d'arrêt telles qu'elles sont écrites, en traitant le contrôleur cible comme le contrôleur « défaillant » dans ce contenu.
* Si le contrôleur cible est *défaillant*, suivez les étapes d'arrêt décrites.
...
== Étape 1 : Arrêter le module de contrôleur cible

Prenez le contrôle et arrêtez le module de contrôleur cible afin que sa paire haute disponibilité continue de fournir des données pendant l'ajout du module d'E/S. Les étapes liées font référence au contrôleur arrêté comme étant le contrôleur « défaillant » ; les mêmes étapes s'appliquent que vous preniez le contrôle d'un contrôleur opérationnel pour un ajout planifié ou que vous arrêtiez un contrôleur déjà défaillant.
