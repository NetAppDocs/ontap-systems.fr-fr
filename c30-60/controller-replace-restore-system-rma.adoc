...
[role="lead"]
Restaurez la configuration NetApp Storage Encryption (si nécessaire), installez les licences pour le contrôleur de remplacement, vérifiez que les interfaces logiques (LIF) indiquent leur port d'origine, effectuez un contrôle de l'état du cluster, enregistrez le numéro de série du contrôleur de remplacement, puis renvoyez la pièce défectueuse à NetApp pour terminer l'étape finale de la procédure de remplacement des contrôleurs AFF C30 et C60.
