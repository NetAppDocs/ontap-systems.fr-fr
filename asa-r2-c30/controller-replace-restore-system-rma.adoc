...
summary: 'Pour terminer le remplacement du contrôleur de votre système de stockage ASA C30, commencez par restaurer la configuration NetApp Storage Encryption si nécessaire et installez les licences requises sur le nouveau contrôleur. Ensuite, vérifiez que les interfaces logiques communiquent avec leurs ports d"attache et effectuez une vérification de l"état du cluster. Enfin, enregistrez le numéro de série du nouveau contrôleur, puis renvoyez la pièce défectueuse à NetApp.' 
---
