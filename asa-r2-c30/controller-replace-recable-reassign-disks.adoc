...
summary: 'Restituez le contrôle des ressources de stockage au contrôleur de remplacement afin que votre système de stockage ASA C30 puisse reprendre son fonctionnement normal. La procédure de restitution varie selon le type de chiffrement utilisé par votre système : (sans chiffrement), avec le gestionnaire de clés intégré (OKM) ou avec le gestionnaire de clés externe (EKM).' 
---
= Remettre le contrôleur - ASA C30
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Renvoyez le contrôle des ressources de stockage au contrôleur de remplacement afin que votre système de stockage ASA C30 puisse reprendre son fonctionnement normal. La procédure de retour à zéro varie en fonction du type de chiffrement utilisé par votre système : pas de chiffrement, chiffrement OKM (Onboard Key Manager) ou chiffrement EKM (External Key Manager).
