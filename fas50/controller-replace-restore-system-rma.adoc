...
summary: 'Pour terminer le remplacement du contrôleur de votre système FAS50, commencez par restaurer la configuration NetApp Storage Encryption si nécessaire et installez les licences requises sur le nouveau contrôleur. Ensuite, vérifiez que les interfaces logiques sont rattachées à leurs ports d"attache et effectuez une vérification de l"état du cluster. Enfin, enregistrez le numéro de série du nouveau contrôleur, puis retournez la pièce défectueuse à NetApp.' 
---
= Remplacement complet du contrôleur - FAS50
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
