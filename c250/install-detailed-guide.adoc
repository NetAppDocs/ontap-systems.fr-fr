...
[role="lead"]
Cette procédure fournit des instructions détaillées étape par étape pour l'installation d'un système de stockage AFF C250.
...
.Étapes
. Câblez les ports d'interconnexion de cluster e0c à e0c et e0d à e0d avec les câbles d'interconnexion de cluster 25 GbE.
+
...
NOTE: link:https://hwu.netapp.com["NetApp Hardware Universe"^]La priorité d’emplacement pour les cartes réseau hôtes (Fibre Channel ou 25 GbE) est l’emplacement 2. Cependant, si vous disposez des deux cartes, la carte Fibre Channel va dans l’emplacement 2 et la carte 25 GbE dans l’emplacement 1 (comme indiqué dans les options ci-dessous). Si vous avez une étagère externe, la carte de stockage va dans l’emplacement 1, le seul emplacement pris en charge pour les étagères.
...
--
.Option 2 : Connexion par câble à un réseau de données ou hôte 25 GbE
--
Les ports 25 GbE des contrôleurs sont connectés à des commutateurs de réseau de données ou de réseau hôte 25 GbE.
...
NOTE: Pour plus d'informations sur la manière d'établir une connectivité continue d'un port de console de contrôleur pour la gestion à distance, consultez l'article de la base de connaissances. https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/How_to_connect_an_ONTAP_controller_console_port_for_remote_management["Comment connecter un port console de contrôleur ONTAP pour la gestion à distance"^].
