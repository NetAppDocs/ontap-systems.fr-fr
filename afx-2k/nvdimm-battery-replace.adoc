...
summary: 'Remplacez la batterie NV de votre système de stockage afx 2k dès qu"elle commence à se décharger ou qu"elle tombe en panne, car elle assure la sauvegarde des données système critiques en cas de coupure de courant. La procédure de remplacement consiste à éteindre le contrôleur défectueux, à retirer le module de contrôleur, à remplacer la batterie NV, à réinstaller le module de contrôleur et à renvoyer la pièce défectueuse à NetApp.' 
---
= Remplacez la pile NV - AFX 2K
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
