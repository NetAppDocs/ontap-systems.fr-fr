...
[role="lead"]
Assurez la maintenance du matériel de votre système de stockage AFX 2K pour garantir sa fiabilité à long terme et des performances optimales. Effectuez des opérations de maintenance régulières, comme le remplacement des composants défectueux, afin de prévenir l'interruption et la perte de données.

Les procédures de maintenance supposent que le système de stockage AFX 2K a déjà été déployé en tant que nœud de stockage dans l'environnement ONTAP.



== Composants du système

Pour le système de stockage AFX 2K, vous pouvez effectuer des procédures de maintenance sur les composants suivants.
...
link:bootmedia-replace-workflow.html["Support de démarrage"]
 a| 
Le support de démarrage stocke un ensemble principal et secondaire de fichiers image ONTAP que le système utilise lors de son démarrage.
...
Un ventilateur refroidit le contrôleur.
 ...
 a| 
link:nvram-replace.html["Module NVRAM"]
 a| 
Le module NVRAM (mémoire vive non volatile) permet au contrôleur de protéger et de sauvegarder les données en vol si le système perd de l'énergie.  L'ID système est stocké dans le module NVRAM .  Lors du remplacement, le contrôleur utilise le nouvel ID système du module NVRAM de remplacement.



 a| 
link:nv12l-replace.html["Module NVRAM12-EX"]
 a| 
Le NVRAM12-EX est le deuxième module NVRAM (Non-Volatile Random Access Memory) pour le contrôleur AFX 2K. Le module NVRAM12-EX permet au contrôleur de protéger et de sauvegarder les données en transit si le système perd l'alimentation.
