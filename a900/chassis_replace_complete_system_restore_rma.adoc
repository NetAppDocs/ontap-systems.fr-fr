...
. Effectuez d'autres vérifications de l'état de santé comme décrit dans https://["Comment_exécuter_une_vérification_de_l'intégrité_d'un_cluster_avec_un_script_dans_ONTAP"^]
...
* Résoudre tout https://["Alertes et risques liés au bien-être Active IQ"^] (Active IQ prendra du temps pour traiter les supports automatiques après la mise sous tension - attendez-vous à un retard dans les résultats)
* Courez https://["Active IQ Config Advisor"^]
* Vérifiez l'état du système à l'aide de https://["Comment_exécuter_une_vérification_de_l'intégrité_d'un_cluster_avec_un_script_dans_ONTAP"^]
