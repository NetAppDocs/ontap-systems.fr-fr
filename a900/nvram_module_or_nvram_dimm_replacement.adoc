...
[role="lead"]
Utilisez la procédure appropriée si le cryptage du stockage est activé.
...
. Saisissez la phrase de passe du gestionnaire de clés intégré pour le cluster à l'invite : exemple requis ici
. Entrez les données de la clé de sauvegarde
+

NOTE: Le nœud revient au menu de démarrage une fois les informations de phrase de passe terminées.
...
. Entrez les informations du certificat de gestion à l'invite : exemple requis ici *****
+
...
--
====
. Réinitialisez le MSID des disques :
+
.. Si vous utilisez le chiffrement de stockage intégré, suivez la procédure décrite dans la section https://docs.netapp.com/us-en/ontap/encryption-at-rest/assign-authentication-keys-seds-onboard-task.html["Attribution d'une clé d'authentification des données à un lecteur FIPS ou SED (gestion des clés intégrée)"^]
.. Je vous utilise un gestionnaire de clés externe, suivez la procédure de la section https://docs.netapp.com/us-en/ontap/encryption-at-rest/assign-authentication-keys-seds-external-task.html["Attribution d'une clé d'authentification de données à un lecteur FIPS ou SED (gestion de clés externe)"^]
