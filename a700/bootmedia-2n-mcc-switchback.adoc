...
summary: 'Une fois le remplacement de l"unité remplaçable sur site (FRU) terminé dans une configuration MetroCluster à deux nœuds, vous pouvez effectuer l"opération de retour en arrière du MetroCluster . Cela rétablit la configuration dans son état de fonctionnement normal, les machines virtuelles de stockage de source de synchronisation (SVM) sur le site précédemment défaillant étant désormais actives et fournissant des données à partir des pools de disques locaux. Le système AFF A700 ne prend en charge que les procédures de récupération manuelles du support de démarrage.' 
---
= Basculement des agrégats dans une configuration MetroCluster à deux nœuds - AFF A700
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Une fois le remplacement du support de démarrage terminé, effectuez l'opération de basculement MetroCluster . Le système AFF A700 ne prend en charge que les procédures de récupération manuelles du support de démarrage. La récupération automatique du support de démarrage n'est pas prise en charge.
