...
summary: 'Avant de mettre hors service le nœud défaillant et de vérifier l"état des clés de chiffrement embarquées, vous devez vérifier l"état du nœud défaillant, désactiver la restitution automatique et vérifier la version d" ONTAP exécutée par le système. Le système AFF A700 ne prend en charge que les procédures de récupération manuelles du support de démarrage.' 
---
...
[role="lead"]
Pour garantir la sécurité des données sur votre système de stockage, vous devez vérifier la prise en charge et l'état de la clé de chiffrement sur votre support de démarrage. Le système AFF A700 ne prend en charge que les procédures de récupération manuelles du support de démarrage. La récupération automatique du support de démarrage n'est pas prise en charge.
