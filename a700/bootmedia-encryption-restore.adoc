...
summary: 'Une fois les variables d"environnement vérifiées, vous devez effectuer des étapes spécifiques aux systèmes sur lesquels Onboard Key Manager (OKM), NetApp Storage Encryption (NSE) ou NetApp Volume Encryption (NVE) sont activés. Le système AFF A700 ne prend en charge que les procédures de récupération manuelles du support de démarrage.' 
---
= Restaurer le chiffrement - AFF A700
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Restaurez le chiffrement sur le support de démarrage de remplacement. Le système AFF A700 ne prend en charge que les procédures de récupération manuelles du support de démarrage. La récupération automatique du support de démarrage n'est pas prise en charge.
