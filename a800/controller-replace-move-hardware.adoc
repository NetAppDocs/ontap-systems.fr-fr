...
[role="lead"]
Remplacez le contrôleur de votre système AFF A800 lorsqu'une panne matérielle l'exige. Le processus de remplacement implique le retrait du contrôleur défectueux, le déplacement des composants vers le contrôleur de remplacement, l'installation du contrôleur de remplacement et son redémarrage.
...
.Et la suite ?
Après avoir remplacé le contrôleur A800 défectueux, vous devez :link:controller-replace-system-config-restore-and-verify.html["restaurez la configuration du système"]
