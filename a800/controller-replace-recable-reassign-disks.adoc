...
---
= Récupération du système et réaffectation des disques - AFF A800
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Rendez le contrôle des ressources de stockage au contrôleur de remplacement afin que votre système AFF A800 puisse reprendre son fonctionnement normal.  La procédure de restitution varie en fonction du type de chiffrement utilisé par votre système : aucun chiffrement ou chiffrement Onboard Key Manager (OKM).
...
Cette étape s'applique uniquement aux systèmes exécutant ONTAP dans une paire HA.
