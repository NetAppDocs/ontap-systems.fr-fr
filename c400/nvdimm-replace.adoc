...
summary: 'Vous devez remplacer la NVDIMM dans le module contrôleur lorsque votre système détecte que la durée de vie de la mémoire flash est presque terminée ou que la NVDIMM identifiée n"est pas en bon état général, ou, si vous ne le faites pas, cela provoquera une panique du système.' 
---
= Remplacez un module NVDIMM - AFF C400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
