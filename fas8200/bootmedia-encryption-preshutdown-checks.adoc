...
summary: 'Vérifiez la prise en charge et l"état de la clé de chiffrement avant d"arrêter le contrôleur défectueux sur un système FAS8200 . Le système FAS8200 ne prend en charge que les procédures de récupération manuelle du support de démarrage. La récupération automatique du support de démarrage n"est pas prise en charge.' 
---
= Vérifier la prise en charge et l'état des clés de cryptage - FAS8200
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Vérifiez la prise en charge et l'état de la clé de chiffrement avant d'arrêter le contrôleur défectueux sur un système FAS8200 . Cette procédure comprend la vérification de la compatibilité de la version ONTAP avec NetApp Volume Encryption (NVE), la vérification de la configuration du gestionnaire de clés et la sauvegarde des informations de chiffrement pour garantir la sécurité des données lors de la récupération du support de démarrage.
