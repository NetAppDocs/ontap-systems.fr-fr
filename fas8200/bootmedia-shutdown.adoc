...
summary: 'Arrêtez le contrôleur défectueux sur un système FAS8200 après avoir terminé les vérifications de cryptage. Le système FAS8200 ne prend en charge que les procédures de récupération manuelle du support de démarrage. La récupération automatique du support de démarrage n"est pas prise en charge.' 
---
= Arrêtez le contrôleur défaillant - FAS8200
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Arrêtez le contrôleur défectueux sur un système FAS8200 après avoir terminé les vérifications de cryptage. Cette procédure comprend l'accès au terminal LOADER, la capture des variables d'environnement de démarrage à titre de référence et la préparation du terminal pour le remplacement du support de démarrage, avec des étapes spécifiques qui varient en fonction de la configuration de votre système.
...
Terminez l'arrêt du contrôleur défectueux en le ramenant à l'invite LOADER et en enregistrant les variables d'environnement de démarrage. L'invite LOADER correspond à l'état d'arrêt sécurisé à partir duquel vous pouvez remplacer le support de démarrage.
