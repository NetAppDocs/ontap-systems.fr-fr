...
... Attendez `savecore` commande à compléter avant d'émettre le retour.
+
Vous pouvez saisir la commande suivante pour contrôler la progression du `savecore` commande : `system node run -node _local-node-name_ partner savecore -s`
