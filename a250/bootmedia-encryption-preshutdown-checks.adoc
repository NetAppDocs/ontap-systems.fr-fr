...
---
= Vérifiez la prise en charge et l'état de la clé de cryptage - AFF A250
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Pour garantir la sécurité des données sur votre système de stockage AFF A250, vous devez vérifier la prise en charge et l’état de la clé de chiffrement sur votre support de démarrage. Vérifiez si votre version d’ONTAP prend en charge NetApp Volume Encryption (NVE), et avant d’éteindre le contrôleur, vérifiez que le gestionnaire de clés est actif.

Si votre système de stockage exécute ONTAP 9.18.1 ou une version ultérieure, utilisez la link:bootmedia-replace-workflow-bmr.html["procédure de récupération de démarrage automatisée"]. Si votre système exécute une version antérieure d'ONTAP, vous devez utiliser le processus de récupération manuelle par démarrage.
