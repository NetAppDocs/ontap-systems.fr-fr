...
summary: 'Vous devez remplacer une barrette DIMM dans le contrôleur lorsque votre système de stockage rencontre des erreurs telles que des erreurs de code de correction d"erreurs corrigibles excessives basées sur des alertes du Health Monitor ou des erreurs ECC non corrigibles, généralement causées par une seule défaillance de DIMM empêchant le système de stockage de démarrer ONTAP.' 
---
