...
summary: 'Une fois les variables d"environnement vérifiées, vous devez effectuer une procédure spécifique aux systèmes sur lesquels OKM (Onboard Key Manager), NetApp Storage Encryption (NSE) ou NetApp Volume Encryption (NVE) sont activés.' 
---
= OKM, NSE et NVE si besoin : AFF A700 et FAS9000
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Si NSE ou NVE sont activés et que le gestionnaire de clés intégré est activé, rendez-vous sur  1: Restore NVE or NSE when Onboard Key Manager is enabled.
* Si NSE ou NVE sont activés pour ONATP 9.5, rendez-vous sur  2: Restore NSE/NVE on systems running ONTAP 9.5 and earlier.
* Si NSE ou NVE sont activés pour ONTAP 9.6, rendez-vous sur le site  3: Restore NSE/NVE on systems running ONTAP 9.6 and later.
