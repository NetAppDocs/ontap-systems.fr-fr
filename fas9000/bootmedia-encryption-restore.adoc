...
summary: 'Une fois les variables d"environnement vérifiées, vous devez effectuer des étapes spécifiques aux systèmes sur lesquels Onboard Key Manager (OKM), NetApp Storage Encryption (NSE) ou NetApp Volume Encryption (NVE) sont activés. Le système FAS9000 ne prend en charge que les procédures de récupération manuelle du support de démarrage.' 
---
