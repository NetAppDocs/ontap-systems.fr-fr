...
summary: 'Une fois le remplacement de l"unité remplaçable sur site (FRU) terminé dans une configuration MetroCluster à deux nœuds, vous pouvez effectuer l"opération de basculement MetroCluster. Cela ramène la configuration à son état de fonctionnement normal, avec les machines virtuelles de stockage SVM source de synchronisation sur le site précédemment défaillant désormais actives et servant les données à partir des pools de disques locaux. Le système FAS9000 prend uniquement en charge les procédures manuelles de récupération du support de démarrage.' 
---
