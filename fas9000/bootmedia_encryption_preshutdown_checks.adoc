...
** Si <lno-DARE> n'est pas affiché dans la sortie de la commande et que le système exécute ONTAP 9.5, passer à  1: Check NVE or NSE on systems running ONTAP 9.5 and earlier.
** Si <lno-DARE> ne s'affiche pas dans la sortie de la commande et si le système exécute ONTAP 9.6 ou une version ultérieure, passer à  2: Check NVE or NSE on systems running ONTAP 9.6 and later.


. Si le nœud douteux est partie d'une configuration HA, désactivez le rétablissement automatique du nœud en bon état : `storage failover modify -node local -auto-giveback false` ou `storage failover modify -node local -auto-giveback-after-panic false`
