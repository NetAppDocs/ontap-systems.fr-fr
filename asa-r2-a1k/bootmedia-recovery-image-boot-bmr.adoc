...
summary: 'Restaurez l"image ONTAP sur le support de démarrage de remplacement de votre système ASA A1K. Le processus implique de lancer le processus de restauration de démarrage à partir de l"invite Loader et de surveiller le processus de restauration jusqu"à ce qu"il soit terminé et que le nœud redémarre.' 
---
= Restaurez l'image sur le support de démarrage - ASA A1K
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Après avoir installé le nouveau périphérique de démarrage sur votre système ASA A1K, vous pouvez lancer le processus de récupération automatique du support de démarrage pour restaurer la configuration à partir du nœud partenaire.
...
** Pour vérifier si OKM est activé, utilisez la commande suivante :
+
`security key-manager onboard show`

** Pour vérifier si EKM est activé, utilisez la commande suivante :
+
...
....
=====
*** Si la clé n'est pas restaurée, le système s'arrête et indique qu'elle n'a pas pu restaurer la clé. Les messages d'erreur et d'avertissement s'affichent. Relancez le processus de récupération en entrant `boot_recovery -partner`.
+
