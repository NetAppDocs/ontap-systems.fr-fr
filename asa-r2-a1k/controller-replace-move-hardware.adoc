...
---
= Remplacer le contrôleur - ASA A1K
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Remplacez le contrôleur de votre système ASA A1K lorsqu'une panne matérielle l'exige. Le processus de remplacement implique le retrait du contrôleur défectueux, le déplacement des composants vers le contrôleur de remplacement, l'installation du contrôleur de remplacement et son redémarrage.
...
.Et la suite ?
Après avoir remplacé le contrôleur ASA A1K défectueux, vous devez link:controller-replace-system-config-restore-and-verify.html["restaurez la configuration du système"].
