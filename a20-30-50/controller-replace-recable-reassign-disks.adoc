...
[role="lead"]
Renvoyez le contrôle des ressources de stockage au contrôleur de remplacement afin que votre système de stockage AFF A20, AFF A30 ou AFF A50 puisse reprendre son fonctionnement normal.  La procédure de restitution varie en fonction du type de cryptage utilisé par votre système : aucun cryptage, cryptage Onboard Key Manager (OKM) ou cryptage External Key Manager (EKM).
